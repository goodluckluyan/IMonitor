...
SOAP_SOURCE_STAMP("@(#) soapC.cpp ver 2.7.15 2014-10-23 10:18:59 GMT")
...
{	switch (p->type)
	...
	case SOAP_TYPE_mons__MontorStateRes:
		if (p->size < 0)
			delete (struct mons__MontorStateRes*)p->ptr;
		else
			delete[] (struct mons__MontorStateRes*)p->ptr;
		break;
	case SOAP_TYPE_mons__GetMontorState:
		if (p->size < 0)
			delete (struct mons__GetMontorState*)p->ptr;
		else
			delete[] (struct mons__GetMontorState*)p->ptr;
		break;
	case SOAP_TYPE_mons__TmsStateRes:
		if (p->size < 0)
			delete (struct mons__TmsStateRes*)p->ptr;
		else
			delete[] (struct mons__TmsStateRes*)p->ptr;
		break;
	case SOAP_TYPE_mons__GetTMSState:
		if (p->size < 0)
			delete (struct mons__GetTMSState*)p->ptr;
		else
			delete[] (struct mons__GetTMSState*)p->ptr;
		break;
	case SOAP_TYPE_std__string:
	...
	case SOAP_TYPE_mons__SMSState:
		if (p->size < 0)
			delete (struct mons__SMSState*)p->ptr;
		else
			delete[] (struct mons__SMSState*)p->ptr;
		break;
	case SOAP_TYPE_mons__GetSMSState:
		if (p->size < 0)
			delete (struct mons__GetSMSState*)p->ptr;
		else
			delete[] (struct mons__GetSMSState*)p->ptr;
		break;
	case SOAP_TYPE_mons__RaidStateRes:
		if (p->size < 0)
			delete (struct mons__RaidStateRes*)p->ptr;
		else
			delete[] (struct mons__RaidStateRes*)p->ptr;
		break;
	case SOAP_TYPE_mons__GetRaidtate:
		if (p->size < 0)
			delete (struct mons__GetRaidtate*)p->ptr;
		else
			delete[] (struct mons__GetRaidtate*)p->ptr;
		break;
	case SOAP_TYPE_mons__ethstate:
		if (p->size < 0)
			delete (struct mons__ethstate*)p->ptr;
		else
			delete[] (struct mons__ethstate*)p->ptr;
		break;
	case SOAP_TYPE_mons__GetEthStateResponse:
		if (p->size < 0)
			delete (struct mons__GetEthStateResponse*)p->ptr;
		else
			delete[] (struct mons__GetEthStateResponse*)p->ptr;
		break;
	case SOAP_TYPE_mons__GetEthState:
		if (p->size < 0)
			delete (struct mons__GetEthState*)p->ptr;
		else
			delete[] (struct mons__GetEthState*)p->ptr;
		break;
	case SOAP_TYPE_mons__SwitchStateRes:
		if (p->size < 0)
			delete (struct mons__SwitchStateRes*)p->ptr;
		else
			delete[] (struct mons__SwitchStateRes*)p->ptr;
		break;
	case SOAP_TYPE_mons__GetSwitchState:
		if (p->size < 0)
			delete (struct mons__GetSwitchState*)p->ptr;
		else
			delete[] (struct mons__GetSwitchState*)p->ptr;
		break;
	case SOAP_TYPE_mons__IngestSpeedLimitRes:
		if (p->size < 0)
			delete (struct mons__IngestSpeedLimitRes*)p->ptr;
		else
			delete[] (struct mons__IngestSpeedLimitRes*)p->ptr;
		break;
	case SOAP_TYPE_mons__GetIngestSpeedLimit:
		if (p->size < 0)
			delete (struct mons__GetIngestSpeedLimit*)p->ptr;
		else
			delete[] (struct mons__GetIngestSpeedLimit*)p->ptr;
		break;
	case SOAP_TYPE_mons__WorkStateRes:
		if (p->size < 0)
			delete (struct mons__WorkStateRes*)p->ptr;
		else
			delete[] (struct mons__WorkStateRes*)p->ptr;
		break;
	case SOAP_TYPE_mons__GetWorkState_USCORECS:
		if (p->size < 0)
			delete (struct mons__GetWorkState_USCORECS*)p->ptr;
		else
			delete[] (struct mons__GetWorkState_USCORECS*)p->ptr;
		break;
	case SOAP_TYPE_mons__ExeSwitchTMSToOtherResponse:
		if (p->size < 0)
			delete (struct mons__ExeSwitchTMSToOtherResponse*)p->ptr;
		else
			delete[] (struct mons__ExeSwitchTMSToOtherResponse*)p->ptr;
		break;
	case SOAP_TYPE_mons__ExeSwitchTMSToOther:
		if (p->size < 0)
			delete (struct mons__ExeSwitchTMSToOther*)p->ptr;
		else
			delete[] (struct mons__ExeSwitchTMSToOther*)p->ptr;
		break;
	case SOAP_TYPE_SOAP_ENV__Header:
	...
	case SOAP_TYPE_std__vectorTemplateOfmons__ethstate:
		if (p->size < 0)
			delete (std::vector<struct mons__ethstate >*)p->ptr;
		else
			delete[] (std::vector<struct mons__ethstate >*)p->ptr;
		break;
	case SOAP_TYPE_std__vectorTemplateOfint:
...
SOAP_FMAC1 struct mons__ExeSwitchTMSToOther * SOAP_FMAC2 soap_instantiate_mons__ExeSwitchTMSToOther(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__ExeSwitchTMSToOther);
		if (size)
			*size = sizeof(struct mons__ExeSwitchTMSToOther);
	}
	else
	{	cp->ptr = (void*)new (struct mons__ExeSwitchTMSToOther[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__ExeSwitchTMSToOtherResponse * SOAP_FMAC2 soap_instantiate_mons__ExeSwitchTMSToOtherResponse(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__ExeSwitchTMSToOtherResponse);
		if (size)
			*size = sizeof(struct mons__ExeSwitchTMSToOtherResponse);
	}
	else
	{	cp->ptr = (void*)new (struct mons__ExeSwitchTMSToOtherResponse[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__GetWorkState_USCORECS * SOAP_FMAC2 soap_instantiate_mons__GetWorkState_USCORECS(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__GetWorkState_USCORECS);
		if (size)
			*size = sizeof(struct mons__GetWorkState_USCORECS);
	}
	else
	{	cp->ptr = (void*)new (struct mons__GetWorkState_USCORECS[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__WorkStateRes * SOAP_FMAC2 soap_instantiate_mons__WorkStateRes(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__WorkStateRes);
		if (size)
			*size = sizeof(struct mons__WorkStateRes);
	}
	else
	{	cp->ptr = (void*)new (struct mons__WorkStateRes[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__GetIngestSpeedLimit * SOAP_FMAC2 soap_instantiate_mons__GetIngestSpeedLimit(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__GetIngestSpeedLimit);
		if (size)
			*size = sizeof(struct mons__GetIngestSpeedLimit);
	}
	else
	{	cp->ptr = (void*)new (struct mons__GetIngestSpeedLimit[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__IngestSpeedLimitRes * SOAP_FMAC2 soap_instantiate_mons__IngestSpeedLimitRes(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__IngestSpeedLimitRes);
		if (size)
			*size = sizeof(struct mons__IngestSpeedLimitRes);
	}
	else
	{	cp->ptr = (void*)new (struct mons__IngestSpeedLimitRes[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__GetSwitchState * SOAP_FMAC2 soap_instantiate_mons__GetSwitchState(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__GetSwitchState);
		if (size)
			*size = sizeof(struct mons__GetSwitchState);
	}
	else
	{	cp->ptr = (void*)new (struct mons__GetSwitchState[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__SwitchStateRes * SOAP_FMAC2 soap_instantiate_mons__SwitchStateRes(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__SwitchStateRes);
		if (size)
			*size = sizeof(struct mons__SwitchStateRes);
	}
	else
	{	cp->ptr = (void*)new (struct mons__SwitchStateRes[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__GetEthState * SOAP_FMAC2 soap_instantiate_mons__GetEthState(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__GetEthState);
		if (size)
			*size = sizeof(struct mons__GetEthState);
	}
	else
	{	cp->ptr = (void*)new (struct mons__GetEthState[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__GetEthStateResponse * SOAP_FMAC2 soap_instantiate_mons__GetEthStateResponse(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__GetEthStateResponse);
		if (size)
			*size = sizeof(struct mons__GetEthStateResponse);
	}
	else
	{	cp->ptr = (void*)new (struct mons__GetEthStateResponse[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__ethstate * SOAP_FMAC2 soap_instantiate_mons__ethstate(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__ethstate);
		if (size)
			*size = sizeof(struct mons__ethstate);
	}
	else
	{	cp->ptr = (void*)new (struct mons__ethstate[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__GetRaidtate * SOAP_FMAC2 soap_instantiate_mons__GetRaidtate(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__GetRaidtate);
		if (size)
			*size = sizeof(struct mons__GetRaidtate);
	}
	else
	{	cp->ptr = (void*)new (struct mons__GetRaidtate[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__RaidStateRes * SOAP_FMAC2 soap_instantiate_mons__RaidStateRes(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__RaidStateRes);
		if (size)
			*size = sizeof(struct mons__RaidStateRes);
	}
	else
	{	cp->ptr = (void*)new (struct mons__RaidStateRes[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__GetSMSState * SOAP_FMAC2 soap_instantiate_mons__GetSMSState(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__GetSMSState);
		if (size)
			*size = sizeof(struct mons__GetSMSState);
	}
	else
	{	cp->ptr = (void*)new (struct mons__GetSMSState[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__SMSState * SOAP_FMAC2 soap_instantiate_mons__SMSState(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__SMSState);
		if (size)
			*size = sizeof(struct mons__SMSState);
	}
	else
	{	cp->ptr = (void*)new (struct mons__SMSState[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__GetTMSState * SOAP_FMAC2 soap_instantiate_mons__GetTMSState(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__GetTMSState);
		if (size)
			*size = sizeof(struct mons__GetTMSState);
	}
	else
	{	cp->ptr = (void*)new (struct mons__GetTMSState[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__TmsStateRes * SOAP_FMAC2 soap_instantiate_mons__TmsStateRes(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__TmsStateRes);
		if (size)
			*size = sizeof(struct mons__TmsStateRes);
	}
	else
	{	cp->ptr = (void*)new (struct mons__TmsStateRes[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__GetMontorState * SOAP_FMAC2 soap_instantiate_mons__GetMontorState(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__GetMontorState);
		if (size)
			*size = sizeof(struct mons__GetMontorState);
	}
	else
	{	cp->ptr = (void*)new (struct mons__GetMontorState[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC1 struct mons__MontorStateRes * SOAP_FMAC2 soap_instantiate_mons__MontorStateRes(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (struct mons__MontorStateRes);
		if (size)
			*size = sizeof(struct mons__MontorStateRes);
	}
	else
	{	cp->ptr = (void*)new (struct mons__MontorStateRes[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
...
SOAP_FMAC3 std::vector<struct mons__ethstate >* SOAP_FMAC4 soap_in_std__vectorTemplateOfmons__ethstate(struct soap *soap, const char *tag, std::vector<struct mons__ethstate >*a, const char *type)
{
	...
	{	soap_revert(soap);
		...
		{
			if (!soap_in_mons__ethstate(soap, tag, &n, "mons:ethstate"))
				break;
		}
		a->push_back(n);
		soap_flag = 1;
...
SOAP_FMAC1 std::vector<struct mons__ethstate > * SOAP_FMAC2 soap_instantiate_std__vectorTemplateOfmons__ethstate(struct soap *soap, int n, const char *type, const char *arrayType, size_t *size)
{
	...
	if (!cp)
		return NULL;
	if (n < 0)
	{	cp->ptr = (void*)new (std::vector<struct mons__ethstate >);
		if (size)
			*size = sizeof(std::vector<struct mons__ethstate >);
	}
	else
	{	cp->ptr = (void*)new (std::vector<struct mons__ethstate >[n]);
		if (!cp->ptr)
		{	soap->error = SOAP_EOM;
			return NULL;
		}
