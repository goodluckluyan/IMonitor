...
SOAP_SOURCE_STAMP("@(#) soapC.cpp ver 2.8.18 2014-10-11 06:26:35 GMT")
...
{
	(void)soap; (void)a; /* appease -Wall -Werror */
	soap_default_std__string(soap, &a->eth);
...
#ifndef WITH_NOIDREF
	(void)soap; (void)a; /* appease -Wall -Werror */
	soap_serialize_std__string(soap, &a->eth);
#endif
}
...
SOAP_FMAC3 int SOAP_FMAC4 soap_out_mons__ethstate(struct soap *soap, const char *tag, int id, const struct mons__ethstate *a, const char *type)
{
	...
	if (soap_element_begin_out(soap, tag, soap_embedded_id(soap, id, a, SOAP_TYPE_mons__ethstate), type))
		return soap->error;
	if (soap_out_std__string(soap, "eth", -1, &a->eth, ""))
		return soap->error;
...
SOAP_FMAC3 struct mons__ethstate * SOAP_FMAC4 soap_in_mons__ethstate(struct soap *soap, const char *tag, struct mons__ethstate *a, const char *type)
{
	...
	if (soap_element_begin_in(soap, tag, 0, type))
		return NULL;
	a = (struct mons__ethstate *)soap_class_id_enter(soap, soap->id, a, SOAP_TYPE_mons__ethstate, sizeof(struct mons__ethstate), soap->type, soap->arrayType);
	if (!a)
		return NULL;
	...
	if (soap->body && !*soap->href)
	{
		for (;;)
		{	soap->error = SOAP_TAG_MISMATCH;
			if (soap_flag_eth && (soap->error == SOAP_TAG_MISMATCH || soap->error == SOAP_NO_TAG))
				if (soap_in_std__string(soap, "eth", &a->eth, "xsd:string"))
				{	soap_flag_eth--;
					continue;
				}
	...
	{	a = (struct mons__ethstate *)soap_id_forward(soap, soap->href, (void*)a, 0, SOAP_TYPE_mons__ethstate, 0, sizeof(struct mons__ethstate), 0, soap_copy_mons__ethstate);
		if (soap->body && soap_element_end_in(soap, tag))
			return NULL;
	}
